...
import {
  creditPackageRoutes,
  creditPackageAdminRoutes,
} from "./routes/credits/packages";
...
const app = new Elysia()
    ...
    swagger({
      documentation: {
        info: {
          ...
          version: "1.0.0",
          description:
            "Payment gateway and API proxy service for Veil platform",
        },
        tags: [
          { name: "Payments", description: "Payment processing endpoints" },
          {
            name: "Webhooks",
            description: "Webhook handlers for payment providers",
          },
          {
            name: "Credits",
            description: "Credit management and purchase endpoints",
          },
          {
            name: "Internal - Credits",
            description: "Internal credit operations",
          },
          { name: "Admin - Credits", description: "Admin credit management" },
          {
            name: "Proxy",
            description: "API proxy endpoints with usage metering",
          },
          { name: "Admin - Proxy", description: "Admin proxy API management" },
          {
            name: "Admin - Pricing",
            description: "Admin pricing model management",
          },
  ...
  .group("/api/v1", (app) => app.use(paymentRoutes).use(webhookRoutes))
...
console.log(
  `💳 Payment Provider: Razorpay ${config.razorpay.keyId ? "✅" : "❌"}`
);
