/*
  Warnings:

  - A unique constraint covering the columns `[fusionAuthId]` on the table `users` will be added. If there are existing duplicate values, this will fail.
  - Made the column `fusionAuthId` on table `users` required. This step will fail if there are existing NULL values in that column.

*/
-- AlterTable
ALTER TABLE "users" ALTER COLUMN "fusionAuthId" SET NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX "users_fusionAuthId_key" ON "users"("fusionAuthId");
