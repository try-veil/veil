...
export default function Monetize() {
  ...
  useEffect(() => {
    if (selectedProject?.hubListing) {
      const { hubListing } = selectedProject;
      setFormData({
        basicPlan: {
          enabled: hubListing?.basicPlan?.enabled ?? true,
          pricePerMonth: hubListing?.basicPlan?.pricePerMonth ?? 0,
          requestQuotaPerMonth:
            hubListing?.basicPlan?.requestQuotaPerMonth ?? 0,
          hardLimitQuota: hubListing?.basicPlan?.hardLimitQuota ?? 0,
        },
        proPlan: {
          enabled: hubListing?.proPlan?.enabled ?? true,
          pricePerMonth: hubListing?.proPlan?.pricePerMonth ?? 0,
          requestQuotaPerMonth: hubListing?.proPlan?.requestQuotaPerMonth ?? 0,
          hardLimitQuota: hubListing?.proPlan?.hardLimitQuota ?? 0,
        },
        ultraPlan: {
          enabled: hubListing?.ultraPlan?.enabled ?? true,
          pricePerMonth: hubListing?.ultraPlan?.pricePerMonth ?? 0,
          requestQuotaPerMonth:
            hubListing?.ultraPlan?.requestQuotaPerMonth ?? 0,
          hardLimitQuota: hubListing?.ultraPlan?.hardLimitQuota ?? 0,
        },
      });
    }
  }, [selectedProject]);

  type PlanType = "basicPlan" | "proPlan" | "ultraPlan";
  type PlanFieldType =
    | "enabled"
    | "pricePerMonth"
    | "requestQuotaPerMonth"
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(value) =>
                      handleChange("basicPlan", "enabled", value)
                    }
                  />
                </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(value) =>
                      handleChange("proPlan", "enabled", value)
                    }
                  />
                </div>
                <CardDescription className="text-xs mt-1">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        handleChange("proPlan", "pricePerMonth", e.target.value)
                      }
                    />
                    <span className="text-sm">$</span>
                  </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(value) =>
                      handleChange("ultraPlan", "enabled", value)
                    }
                  />
                </div>
                <CardDescription className="text-xs mt-1">
