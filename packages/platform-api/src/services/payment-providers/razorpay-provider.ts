...
export class RazorpayProvider implements PaymentProvider {
  ...
  async processPayment(data: ProcessPaymentProviderRequest): Promise<PaymentProviderResponse> {
    try {
      ...
      const { paymentId, paymentToken } = data;

      // In Razorpay flow:
      // 1. Frontend sends razorpay_payment_id, razorpay_order_id, razorpay_signature
      // 2. We verify the signature
      // 3. If valid, payment is already captured (auto-capture enabled)

      // For now, we'll fetch payment details to confirm
      const payment = await this.razorpay.payments.fetch(paymentToken);
